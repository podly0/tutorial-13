...
window.addEventListener("load", function() {
   ...
   for (var i = 0; i < planOptions.length; i++) {
      planOptions[i].oneclick = calcOrder;
   }
...
function calcOrder() {
   ...
   orderForm.elements.initialCost.value = formatUSCurrency(intialCost);

   //Retrieve the cost of the user's protection plan
   var pCost = document.querySelector('input[name="protection"]: checked').value*querySelector;
   ...
   var salesTax = 0.5*(initialCost + pCost);
   orderForm.elements.subtotal.value = formatNumber(salesTax, 2);
